...
## 1b. WHAT ACTUALLY SHIPPED ON THIS BRANCH

**Phase 1 is complete, end to end.** Every layer in its table exists, builds, and
is covered by tests that go through the live path rather than around it.

| Layer | Landed as |
|---|---|
| Recovered scrap table | `Multiplayer/Loot/loot-scrap-tiers.txt` (133 rows) + `LootScrapTable`, generated by `tools/world-import/generate-loot-scrap-tiers.py`, drift-checked against `itemData.json` by `LootScrapTableIntegrityTests` |
| Budget | `Multiplayer/Loot/LootBudget.cs` — retail's `DoMath` transcribed, constants labelled |
| Roll | `Multiplayer/Loot/LootTable.cs` — deterministic, FNV over the container key |
| Ledger | `Multiplayer/Loot/LootContainerLedger.cs` |
| Identity + grounding | `Multiplayer/LootContainers.cs` (prefab `LootChest_001`, 0.225 m sink) |
| Haven placement | `Resources/HavenSurface.cs` — `LootConfig()`, `LootLocals()`, 10 seats |
| Release placement | `Islands/release-loot-placements.json` (**2,243 seats over 254 islands**) + `ReleaseLootCatalog` + `ReleaseWorldLoot`, generated by `tools/world-import/generate-release-loot-placements.py` |
| Streaming | `ResourceInterestPolicy` gained `loot-` |
| Spawn | `WorldEntities.LootContainerEntity`, gate `WAREBORN_SPAWN_LOOT=1`, count knob `WAREBORN_LOOT_COUNT` (Haven only) |
| Activation | `WorldResourceActivation` loot branch, keyed on the key like the fuel canister |
| Serve | `ComponentsSerializer` 1210 (`Inventory` verb) + 1081 bound through `Game/Loot/LootStock.cs` |
| Open | `Game/Loot/LootService.cs` → `Interact` echo on the container's 1210 |
| Take | `Multiplayer/Inventory/CrossInventoryPolicy.cs` + `crossInventoryMoveItem` / `moveAll` in `InventoryModificationState_Handler` |
| Admin map | `Web/Assets/admin-map-loot.js`, admin-only; `IslandResourceInventory.LootContainers` is a real number with `WarebornTuning` provenance |

**Gates.** `WorldsAdriftRebornGameServer.Multiplayer.Tests` 3,745 passed / 0
(baseline was 3,694). `WorldsAdriftServer.Tests` 1,114 passed / 26 skipped
(baseline 1,107 / 26; the delta is this branch's own tests plus two rewritten
pins). Both server projects build.

**The guard tests were mutation-checked, not assumed.** Deleting the `loot-`
prefix from `ResourceInterestPolicy` and the release registration block from
`WorldEntities.Default` turns exactly two tests red
(`EveryContainerKeyIsStreamedAndActivated`,
`EnablingLootPlacesContainersOnEveryReleaseIsland`) and leaves the rest green.
That is the property the tree work lacked when it shipped invisible.

**Not done, and why.** Phases 2–5 are untouched. Phase 2 is next and its second
half needs the migration flagged below; Phase 4 is not this branch's to write.
