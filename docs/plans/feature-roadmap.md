...
7. **Wind walls are nearly free once cells exist.** ⚠ **WRONG — corrected in
   §12.6a (2026-08-20).** They are nearly free NOW: `WallSegmentVisualizer` has
   one `[Require]` (`1204`) and `GetWeatherAt` lerps wall wind over cell wind, so
   no lattice is involved. They must ship WITH a complete `1229
   GlobalWallDataState` or every wall goes dead calm. `5129` is also named below
   as the per-entity delivery mechanism and is **not** one — it is a
   worker-side report channel with no client reader. See §12.6a.
