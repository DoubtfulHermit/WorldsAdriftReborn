...
(function(){
  ...
  function renderStaticWorldMap(){
    ...
    (worldMap.biomes||[]).forEach(function(b,index){
      ...
      var label=svgEl('text',{x:b.x,y:-b.z-90,'class':'map-cell-label'+(hasDistrict?'':' unassigned')});
  ...
  function renderLiveWorldMap(reporting,ageSeconds){
    ...
    text('mapStatus','Release MapFile · '+(worldMap.islands||[]).length+' islands · '+(worldMap.biomes||[]).length+' tier cells ('+namedCells+' named, '+((worldMap.biomes||[]).length-namedCells)+' unassigned) · '+(worldMap.walls||[]).length+' wall segments · '
